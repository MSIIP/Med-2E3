...
CUDA_VISIBLE_DEVICES=3 python src/inference.py \
    ...
    --llm_dtype bfloat16 \
    --data_path path/to/ctrate/valid_short.json \
    --conv_version phi \
    --vision2d_data_path / \
...
CUDA_VISIBLE_DEVICES=3 python scripts/eval/utils/eval_ctrate_text.py \
    --answer_path path/to/ctrate/valid_short.json \
    --predict_path $MODEL_PATH/eval/valid_short.json \
    --result_path $MODEL_PATH/eval/valid_short_results.json
