...
CUDA_VISIBLE_DEVICES=2 python src/inference.py \
    ...
    --llm_dtype bfloat16 \
    --data_path path/to/ctrate/valid_rg.json \
    --conv_version phi \
    --vision2d_data_path / \
...
CUDA_VISIBLE_DEVICES=2 python scripts/eval/utils/eval_ctrate_text.py \
    --answer_path path/to/ctrate/valid_rg.json \
    --predict_path $MODEL_PATH/eval/valid_rg.json \
    --result_path $MODEL_PATH/eval/valid_rg_results.json
